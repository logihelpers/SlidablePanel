...
class _SlidablePanelControlState extends State<SlidablePanelControl>
  ...
  void initState() {
    ...
    _contentHidden = widget.control.attrBool("content_hidden", true)!;
    ...
    widget.backend.subscribeMethods(widget.control.id, (methodName, args) async {
      switch (methodName) {
        case "toggle_content":
          _togglePanel(bool.parse(args["content_hidden"].toString()));
          break;
      }
      return null;
    });
  ...
  void _togglePanel(bool contentHidden) {
    ...
    widget.backend.updateControlState(widget.control.id, {"content_hidden": contentHidden.toString()});
  }
